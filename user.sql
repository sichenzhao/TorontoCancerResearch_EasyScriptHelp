...
CREATE TABLE final_result AS SELECT chromosome, chromosome_start, chromosome_end, chromosome_strand, assembly_version, reference_genome_allele, mutated_from_allele, mutated_to_allele, count() AS patient_number, max(donor_age_at_enrollment) AS max_donor_age_at_enrollment, min(donor_age_at_enrollment) AS min_donor_age_at_enrollment, avg(donor_age_at_enrollment) AS avg_donor_age_at_enrollment, tumour_grade, donor_tumour_stage_at_diagnosis FROM big_comb GROUP BY chromosome, chromosome_start, chromosome_end, chromosome_strand, assembly_version, reference_genome_allele, mutated_from_allele, mutated_to_allele ORDER BY patient_number DESC;
